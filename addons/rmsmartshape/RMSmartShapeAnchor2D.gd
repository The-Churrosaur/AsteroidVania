...
func _set_monitored_shape(value):
	...
	if value != null:
		if has_node(value):
			...
			connected = get_node(value).is_connected(
				"points_modified", self, "_handle_point_change"
			)
...
func refresh():
	if monitored_shape != null:
		if has_node(monitored_shape) == true:
			...
			if control_point_offset >= 0:
				pt_a_handle = node.global_transform.xform(
					node.get_point_position(pt_a_index) + node.get_point_out(pt_a_index)
				)
				pt_b_handle = node.global_transform.xform(
					node.get_point_position(pt_b_index) + node.get_point_in(pt_b_index)
				)
				...
				n_pt_a = _cubic_bezier(
					...
					pt_b_handle,
					pt_b,
					clamp(control_point_offset - 0.1, 0.0, 1.0)
				)
				...
				pt_a_handle = node.global_transform.xform(
					node.get_point_position(pt_a_index) + node.get_point_in(pt_a_index)
				)
				pt_b_handle = node.global_transform.xform(
					node.get_point_position(pt_b_index) + node.get_point_out(pt_b_index)
				)
